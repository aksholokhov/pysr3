...
class TestLinearLMEOracleW(TestCase):
    ...
    def test_drop_matrices(self):
        ...
        for t, (random_beta, random_gamma) in enumerate(zip(np.random.rand(trials, problem.num_fixed_effects),
            ...
            w_beta = oracle.drop_matrix_beta
            w_gamma = oracle.drop_matrix_gamma
            for j in range(problem.num_fixed_effects):
                ...
                if idx >= 0:
                    ...
                    self.assertTrue(np.isclose(loss - loss3, w_gamma[idx], rtol=rtol, atol=atol),
                ...
                self.assertTrue(np.isclose(loss - loss2, w_beta[j], rtol=rtol, atol=atol),
